...
#define UCT_ROCM_COPY_IFACE_LATENCY  ucs_linear_func_make(8e-6, 0)
...
uct_rocm_copy_estimate_perf(uct_iface_h tl_iface, uct_perf_attr_t *perf_attr)
{
    ...
    if (perf_attr->field_mask & UCT_PERF_ATTR_FIELD_BANDWIDTH) {
        ...
        if (!(perf_attr->field_mask & UCT_PERF_ATTR_FIELD_OPERATION)) {
            perf_attr->bandwidth.shared = 10000 * UCS_MBYTE;
        } else {
            ...
            case UCT_EP_OP_PUT_ZCOPY:
                ...
                break;
            default:
                perf_attr->bandwidth.shared = 10000.0 * UCS_MBYTE;
                break;
            }
        }
    }
    ...
    if (perf_attr->field_mask & UCT_PERF_ATTR_FIELD_LATENCY) {
        perf_attr->latency = UCT_ROCM_COPY_IFACE_LATENCY;
    }
